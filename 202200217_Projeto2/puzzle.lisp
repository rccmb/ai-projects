...
(defun change-board (holes-list b initial-line)
    ...
    (let* 
      ...
      (cond 
        ...
        ((and (null next) (or (= (+ current-cell 1) 1) (= (+ current-cell 1) 3) (= (+ current-cell 1) 5)) (not (= line initial-line))) ; Final is 1, 3 or 5, to 0, if the line is not the starting line.
          (progn
            (if (= initial-line 1) ; Sets the score.
              (set-player-one-score (+ (get-player-one-score) (+ current-cell 1)))
              (set-player-two-score (+ (get-player-two-score) (+ current-cell 1)))
            ) 
            (replace-value line column b 0)
          )
        ) 
        (t (change-board (cdr holes-list) (increment-value line column b) initial-line)) ; Go to the next hole with the new board.
      )
    )
  )
)
  ...
  (if (and (valid-linep line-index) (valid-line-indexp position-index) (not (null board)))
    (let* 
      ...
      (if (= total-pieces 0)
        ...
        (replace-value line-index position-index (change-board holes board line-index) 0) ; Turns the moved value to zero and returns the new board.
      )
    )
    nil
  )
